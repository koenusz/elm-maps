module Maps.Internal.Map exposing
    ...
    , Transformation
    , transformationStyle
    , moveTo, setHeight, setTileServer, setTileSize, setWidth, setZoom
    )
...
setTileServer tileServer map =
    { map | tileServer = tileServer }
...
setHeight height map =
    { map | height = height }


setTileSize : Float -> Map -> Map
setTileSize tileSize map =
...
move thisoffset map =
    let
        centerOffset otheroffset =
            Screen.Offset (map.width / 2 - otheroffset.x) (map.height / 2 - otheroffset.y)
    in
...
setZoom zoomLevel map =
    { map | zoom = min 19 <| max 0 <| zoomLevel }
...
viewBounds bounds map =
    let
        thiszoom =
            Bounds.zoom map.tileSize map.width map.height bounds
    in
...
tiles map =
    let
        ...
        tile =
            Tile.fromLatLng (toFloat <| ceiling map.zoom) map.center
